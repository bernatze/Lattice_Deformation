...
def Differential_force(F0,K,damping,dt, Nod, Link):
    ...
    for node in Nod:
        ...
        if node.isFixed == 0:# and node.isKilled == 0:
...
def Differential_force_point(F0,K,damping,dt, Nod, Link, L):
    ...
    for node in Nod:
        ...
        if node.isFixed == 0 and node.is_aspired==0:# and node.isKilled == 0:
...
def Differential_Relaxation_X_Fix(K,damping,dt, Nod, Link):
    ...
    for node in Nod:
        ...
        if (node.isFixed == 0) and (node.isTop==0):# and node.isKilled == 0:
...
def Deformation_of_rest_lengths_uniform_all(Nod, p):
    ...
    for node in range(0, len(Nod)):
        for neighbour in range (0, len(Nod[node].neighbours)):
            R=random.random()
            if R<p:# and Nod[node].lengths[neighbour]>2:# and Nod[node].rest_lengths[neighbour]<Nod[node].lengths[neighbour]:
...
#            node.Geo_position[0]=node.initial_position[0]
...
############################################################################
############################################################################
######################### USED IN THE PAST NOT USED SO FAR #################
############################################################################
############################################################################
############################################################################
############################################################################
...
################################################################
###### Rest lengths irreversibly updated at random             #
################################################################

def constant_force_y_u_deformation_all(Nod, Link,L,F0,K,Time,dt,damping,p):
    
#    Nod, Link=Struct_Lattice(g,L)
...
def Displacement_Top_Layer_deformation(Nod, Link,L,Displacement,K,Time,dt,damping,p):
    #### TEST
#    p=p*(3*len(Nod))/len(Link)
#    print(p*len(Link))
    #### TEST
...
################################################################
###### Rest lengths updated if stretchingh>Threshold           #
################################################################

def constant_force_y_stretch_deformation(g,L,F0,K,Time,dt,damping,p, Threshold):
...
###################################################################
###### Rest lengths updated if stretchingh>Threshold and uniform  #
###################################################################

def constant_force_y_stretch_deformation_u(g,L,F0,K,Time,dt,damping,p, Threshold):
...
################################################
###### Purely elastic network with lengthmax and bond breaking
################################################
    
def constant_force_y_breaking(g,L,F0,K,Time,dt,damping):
    
    l0=2 #rest length springs
    ...
    for T in range(0,Time):
        ...
        for links in Link:
            ...
            j=links.Topo_link[1]
            length_link=0
            length_link=np.array(Nod[i].Geo_position)-np.array(Nod[j].Geo_position)
            
            if np.sqrt(length_link[0]*length_link[0]+length_link[1]*length_link[1])>2.0*l0:
                links.isKilled=1
                
                ##### Remove node j from the neighbours of i
                T_Nod=[]
                T_Nod_Geo=[]
                for r in range(0, len(Nod[i].neighbours)):
                    if Nod[i].neighbours[r]!=j:
                        u=Nod[i].neighbours[r]
                        T_Nod.append(u)
                        T_Nod_Geo.append(Nod[u].Geo_position)
                Nod[i].neighbours=T_Nod
                Nod[i].neighbours_Geo=T_Nod_Geo
                
                ##### Remove node i from the neighbours of j
                
                T_Nod=[]
                T_Nod_Geo=[]
                for r in range(0, len(Nod[j].neighbours)):
                    if Nod[j].neighbours[r]!=i:
                        u=Nod[j].neighbours[r]
                        T_Nod.append(u)
                        T_Nod_Geo.append(Nod[u].Geo_position)
                Nod[j].neighbours=T_Nod
                Nod[j].neighbours_Geo=T_Nod_Geo
...
def Deformation_of_rest_lengths_uniform(Nod, p):
    R=random.random()
    if R<p:
        node=random.randint(0,len(Nod)-1)
        if len(Nod[node].neighbours)>0:
            neighbour=random.randint(0, len(Nod[node].neighbours)-1)
            if Nod[node].rest_lengths[neighbour]<Nod[node].lengths[neighbour]:
                Nod[node].rest_lengths[neighbour]=Nod[node].lengths[neighbour]
                for k in range(0, len(Nod[neighbour].neighbours)):
                    if Nod[neighbour].neighbours[k]==node:
                        Nod[neighbour].rest_lengths[k]=Nod[node].lengths[neighbour]
